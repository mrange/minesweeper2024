...
"#version 430\n"
 ...
 "}"
 "vec3 n(vec2 v,vec3 y,vec3 f,float a,float m)"
 "{"
   ...
   "x.y=s.y-.5;"
   "x=abs(x);"
   "vec3 z=(i[int(floor(m))]&1<<int(dot(vec2(1,-1)/sqrt(2),x)>0?"
     "3+(l.x+1)/2+l.y+1:"
     "-n))==0?"
     "f:"
     "y;"
   "return z*smoothstep(a,-a,min(p(r,d),p(s.yx-.5,d)));"
 "}"
 "void main()"
 "{"
       ...
       "for(int Z=0;Z<2;++Z)"
         "{"
           "float _=Z==0?"
             ...
             "M.x,b=Z==0?"
             "1-T:"
             "T,h=abs(length(i)-.1*max(S,U));"
           "vec2 j=r[int(_)];"
           "for(float k=0;k<j.y;++k)"
             "h=min(abs(h-.1),h);"
           "vec3 k=(.2+t(2-_))*(j.x*.005/max(h,.003));"
           "if(_<1)"
             "{"
               "vec2 q=i/.25;"
               "q.x+=-q.y/8;"
